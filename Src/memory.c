...
//void* g_memory = (void*)0xC0000000;
void* g_memory = (void*)0x00100000;
//void* g_memory = (void*)0x01000000;

void k_init_mem() 
{
    void* ptr = g_memory; 
    size_t size = MAX_MEM_SIZE;
    ...
    printf("[INIT MEMORY] %d KB\n", (int)(MAX_MEM_SIZE/1024));
}
...
{
    mem_t mem = memget();
    printf("Lower memory: %d KB\0",mem.lower_size);
    printf("Upper memory: %d KB");

    mem = upper_memget();
    printf("Extended 1: %d KB\0", mem.lower_size);
    printf("Extended 2: %d KB\0", mem.upper_size * 64);
}
